...
{
public:
	virtual EGCResults SendMessage(std::uint32_t uMsgType, const void* pData, std::uint32_t uData) = 0;
	virtual bool IsMessageAvailable(std::uint32_t* puMsgSize) = 0;
	virtual EGCResults RetrieveMessage(std::uint32_t* puMsgType, void* pDest, std::uint32_t uDest, std::uint32_t* puMsgSize) = 0;
};
